...
<div x-data="{ spotModal: false, defaultModal: false }" class="space-y-6">
    ...
    <div class="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <form action="{{ route('fines.index') }}" method="GET" class="w-full md:w-auto flex flex-wrap items-center gap-3">
            ...
            <select name="fine_type" class="custom-select h-10 px-3 border border-slate-300 rounded-xl text-xs shadow-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 bg-white text-slate-700">
        ...
        <div class="flex items-center space-x-2">
            <button @click="spotModal = true" type="button" class="h-10 px-4 bg-amber-600 hover:bg-amber-500 text-white font-bold rounded-xl text-xs shadow-md shadow-amber-600/10 transition-all whitespace-nowrap">
                + Issue Spot Fine
            </button>
            <button @click="defaultModal = true" type="button" class="h-10 px-4 bg-rose-600 hover:bg-rose-500 text-white font-bold rounded-xl text-xs shadow-md shadow-rose-600/10 transition-all whitespace-nowrap">
                + Issue Default Fine
            </button>
        </div>
        @endif
            ...
            <table class="min-w-full divide-y divide-slate-200 text-left text-xs">
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <span class="px-2.5 py-1 text-[10px] font-extrabold uppercase rounded-full {{ $fine->status === 'paid' ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-rose-100 text-rose-800 border border-rose-200' }}">
                        ...
                        <td class="px-6 py-4 text-right">
                            ...
                            <form action="{{ route('fines.double', $fine) }}" method="POST" onsubmit="return confirm('Double remaining unpaid balance (Rs {{ number_format($fine->remaining_amount, 2) }} -> Rs {{ number_format($fine->remaining_amount * 2, 2) }})?');">
                                @csrf
                                <button type="submit" class="h-8 px-2.5 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-300 rounded-lg font-bold text-xs transition-colors">
    ...
    <!-- Spot Fine Modal -->
    <div x-show="spotModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
        <div class="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4 border border-slate-200">
            ...
            <form action="{{ route('fines.spot') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="member_id" required class="custom-select w-full h-10 px-3 border border-slate-300 rounded-xl text-xs bg-white text-slate-800">
                ...
                <div>
                    ...
                    <input type="text" name="reason" required placeholder="e.g. Late Arrival to Assembly" class="w-full h-10 px-3 border border-slate-300 rounded-xl text-xs">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Spot Fine Amount (Rs) *</label>
                    <input type="number" name="amount" value="50" min="1" step="1" required class="w-full h-10 px-3 border border-slate-300 rounded-xl text-xs font-bold">
                ...
                <div class="flex items-center justify-end space-x-2 pt-2 border-t border-slate-100">
                    ...
                    <button type="submit" class="h-9 px-5 bg-amber-600 hover:bg-amber-500 text-white font-bold text-xs rounded-xl shadow-md">Issue Spot Fine</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <!-- Default Fine Modal -->
    <div x-show="defaultModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
        <div class="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4 border border-slate-200">
            <div class="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            <form action="{{ route('fines.default') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="reason" required placeholder="e.g. Failure to perform duty" class="w-full h-10 px-3 border border-slate-300 rounded-xl text-xs">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Default Fine Amount (Rs) *</label>
                    <input type="number" name="amount" value="100" min="1" step="1" required class="w-full h-10 px-3 border border-slate-300 rounded-xl text-xs font-bold">
                ...
                <div>
                    ...
                    <textarea name="description" rows="2" class="w-full p-2.5 border border-slate-300 rounded-xl text-xs"></textarea>
                </div>
                <div class="flex items-center justify-end space-x-2 pt-2 border-t border-slate-100">
                    <button @click="defaultModal = false" type="button" class="h-9 px-4 text-xs font-semibold text-slate-500">Cancel</button>
                    <button type="submit" class="h-9 px-5 bg-rose-600 hover:bg-rose-500 text-white font-bold text-xs rounded-xl shadow-md">Issue Default Fine</button>
                </div>
            </form>
        </div>
    </div>
