...
<div class="max-w-3xl mx-auto bg-white p-6 sm:p-8 rounded-2xl border border-slate-200/80 shadow-sm space-y-6">
    <div class="flex items-center justify-between border-b border-slate-100 pb-4">
        <h2 class="text-base font-bold text-slate-900">Edit Member Profile</h2>
    ...
    <form action="{{ route('members.update', $member) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="address" rows="2" class="w-full p-3 border border-slate-300 rounded-xl text-xs focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500">{{ old('address', $member->address) }}</textarea>
        </div>

        <div class="flex items-center justify-end space-x-3 border-t border-slate-100 pt-4">
            <a href="{{ route('members.show', $member) }}" class="h-10 px-4 flex items-center text-xs font-semibold text-slate-500 hover:text-slate-800">Cancel</a>
