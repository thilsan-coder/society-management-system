...
<div class="max-w-3xl mx-auto bg-white p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
    <div class="flex items-center justify-between border-b border-slate-100 pb-4">
        <div>
            <h2 class="text-lg font-bold text-slate-900">Create New Member & Account</h2>
    ...
    <form action="{{ route('members.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="address" rows="2" class="w-full p-3 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500">{{ old('address') }}</textarea>
        </div>

        <div class="flex items-center justify-end space-x-3 border-t border-slate-100 pt-4">
            <a href="{{ route('members.index') }}" class="h-10 px-4 flex items-center text-sm font-semibold text-slate-500 hover:text-slate-800">Cancel</a>
