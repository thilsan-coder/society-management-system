...
<div class="space-y-6">
    ...
    <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <form action="{{ route('meetings.index') }}" method="GET" class="w-full md:w-auto flex flex-wrap items-center gap-3">
            ...
            <a href="{{ route('meetings.index') }}" class="h-10 px-3 flex items-center text-xs font-semibold text-slate-500 hover:text-slate-700">Reset</a>
            @endif
        </form>

        @can('create', App\Models\Meeting::class)
        <a href="{{ route('meetings.create') }}" class="h-10 px-4 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-lg text-sm shadow flex items-center transition-colors whitespace-nowrap">
            + Schedule Meeting
        </a>
        @endcan
    </div>
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="px-6 py-4 text-right">
                            <a href="{{ route('meetings.show', $meeting) }}" class="h-8 px-3 inline-flex items-center bg-teal-50 hover:bg-teal-100 text-teal-700 rounded font-bold text-xs">
                                View Details ↗
                            </a>
                        </td>
                    </tr>
