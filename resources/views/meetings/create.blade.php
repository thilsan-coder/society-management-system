...
<div class="max-w-3xl mx-auto bg-white p-6 sm:p-8 rounded-2xl border border-slate-200/80 shadow-sm space-y-6">
    <div class="flex items-center justify-between border-b border-slate-100 pb-4">
        <h2 class="text-base font-bold text-slate-900">Schedule New Society Meeting</h2>
    ...
    <form action="{{ route('meetings.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="discussed_topics" rows="3" class="w-full p-3 border border-slate-300 rounded-xl text-xs focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500">{{ old('discussed_topics') }}</textarea>
        </div>

        <div>
            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Decisions & Resolutions</label>
            <textarea name="decisions_resolutions" rows="3" class="w-full p-3 border border-slate-300 rounded-xl text-xs focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500">{{ old('decisions_resolutions') }}</textarea>
        </div>

        <div>
            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Action Items & Tasks</label>
            <textarea name="action_items" rows="3" class="w-full p-3 border border-slate-300 rounded-xl text-xs focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500">{{ old('action_items') }}</textarea>
        </div>
