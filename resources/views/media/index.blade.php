...
<div class="space-y-6">
    ...
    <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <form action="{{ route('media.index') }}" method="GET" class="w-full md:w-auto flex items-center gap-3">
            <select name="media_type" class="h-10 px-3 border border-slate-300 rounded-lg text-sm bg-white">
        ...
        <a href="{{ route('media.create') }}" class="h-10 px-4 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-lg text-sm shadow flex items-center transition-colors whitespace-nowrap">
            + Create Poster / Media
        </a>
        @endcan
    </div>

    <!-- Grid / Table (5 per page) -->
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="px-6 py-4 text-right">
                            <a href="{{ route('media.show', $media) }}" class="h-8 px-3 inline-flex items-center bg-teal-50 hover:bg-teal-100 text-teal-700 rounded font-bold text-xs">
                                View Content ↗
                            </a>
                        </td>
                    </tr>
